...
def loadDatatime(args):
    # Traffic
    df = pd.read_hdf(args.traffic_file)
    tmp = df.values
    Traffic = np.concatenate([tmp[i:i+288*5,...] for i in range(0,34272,288*7)],0)
    # train/val/test 
    ...
    timeofday = (Time.hour * 3600 + Time.minute * 60 + Time.second) \
                // 300
    timeofday = np.reshape(timeofday, newshape = (-1, 1))    
    tmp = np.concatenate((dayofweek, timeofday), axis = -1)
    Time = np.concatenate([tmp[i:i+288*5,...] for i in range(0,34272,288*7)],0)
    # train/val/test
    train = Time[: train_steps]
    val = Time[train_steps : train_steps + val_steps]
    test = Time[-test_steps :]
